...
glm::vec3 lightPos(-0.5f, 0.0f, -3.0f);
...
#pragma region object_loop
		...
		GLint torchpLoc = glGetUniformLocation(shaderProgram, "torch.position");
		GLint torchdLoc = glGetUniformLocation(shaderProgram, "torch.direction");
		GLint torchcLoc = glGetUniformLocation(shaderProgram, "torch.cutOff");
		GLint torchoLoc = glGetUniformLocation(shaderProgram, "torch.outerCutOff");

		glUniform3f(torchpLoc, cameraPos.x, cameraPos.y, cameraPos.z);
		glUniform3f(torchdLoc, cameraFront.x, cameraFront.y, cameraFront.z);
		glUniform1f(torchcLoc, glm::cos(glm::radians(12.5f)));
		glUniform1f(torchoLoc, glm::cos(glm::radians(14.5f)));
